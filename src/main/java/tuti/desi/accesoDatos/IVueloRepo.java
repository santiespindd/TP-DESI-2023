...
public interface IVueloRepo extends JpaRepository<Vuelo, Long> {
	boolean existsByFechaPartidaAndAvionId(Date fechaPartida, Long avionId);
	...
	@Query("SELECT v FROM Vuelo v WHERE v.fechaPartida >= :fecha "
	            ...
	            + "AND (:tipoVuelo IS NULL OR v.tipoVuelo = :tipoVuelo) " + "ORDER BY v.fechaPartida ASC")
	    List<Vuelo> consultarVuelos(@Param("fecha") Date fecha,
